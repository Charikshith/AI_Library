...
def parse_tools(md: str):
    ...
    for raw in section.splitlines():
        ...
        tools.append({
            ...
            "stars": cells[4].strip() if len(cells) > 4 else "",
        })
    return tools
...
def tool_card(t):
    ...
    stars = t.get("stars", "")
    stars_html = f'<span class="feed-stars">&#9733; {int(stars):,}</span>' if stars.isdigit() else ''
    return ("".join([
        f'<a class="feed-card" href="{h(t["url"])}"{tgt}>',
        f'<div class="feed-top"><span class="feed-title">{h(t["name"])}</span>',
        f'<span class="feed-meta">{stars_html}<span class="feed-date">{h(t["added"])}</span></span></div>',
